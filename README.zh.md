# 🛰️ Claude Code Status Line — 82 主题、可配置模块、CLI

> 为 Claude Code 打造的精致、可定制状态栏 —— 上下文进度、速率限制、费用、模型等信息全部呈现在一行优雅的彩色文本中。82 个主题和 26 个模块库，可自由组合专属状态栏。
...
82 个主题 × 2 个变体（`detailed` + `-compact`）。使用 `~/.claude/status-line.sh use <name>` 应用。

### 🔝 Top 10 — 最受欢迎，跨文化
`cyberpunk` · `hacker` · `dragonball` · `naruto` · `pokemon` · `ironman` · `spiderman` · `einstein` · `tesla` · `ferrari`

### 🛠 经典（21）：`minimal` `developer` `muted` `mono` `hard-worker` `time` `zen` `rainbow` `anime` `love` `cat` `christmas` `space` `retro` `fire` `ocean` `weather` `coffee` `music` `game` `pirate`
