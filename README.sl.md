# 🛰️ Claude Code Status Line — 82 tem, nastavljivi bloki, CLI

> Dovršena, prilagodljiva statusna vrstica za Claude Code — kontekst, omejitve, stroški, model in več — vse v eni lepo obarvani vrstici. 82 tem in knjižnica 26 blokov za sestavljanje lastne vrstice.
...
82 tem × 2 varianti (`detailed` + `-compact`). Uporabite z `~/.claude/status-line.sh use <name>`.

### 🔝 Top 10 — najbolj iskane, mednarodne
`cyberpunk` · `hacker` · `dragonball` · `naruto` · `pokemon` · `ironman` · `spiderman` · `einstein` · `tesla` · `ferrari`

### 🛠 Klasika (21): `minimal` `developer` `muted` `mono` `hard-worker` `time` `zen` `rainbow` `anime` `love` `cat` `christmas` `space` `retro` `fire` `ocean` `weather` `coffee` `music` `game` `pirate`
...
| Pripravljene teme | ❌ | ✅ 82 tem × 2 različici = **164 pripravljenih** |
