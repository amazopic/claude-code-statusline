# 🛰️ Claude Code Status Line — 82 теми, налаштовувані блоки, CLI

> Гарний, легко розширюваний статусний рядок для Claude Code — контекст, ліміти, вартість, модель та інше — все в одному кольоровому рядку. 82 теми і бібліотека з 26 блоків для збирання власного рядка.
...
Лічильники 5h / 7d містять живий зворотний відлік до моменту, коли скидається кожне вікно: `5h{1h 6m}: 1%` — 5-годинне вікно скидається через 1 годину 6 хвилин; `7d{1d 2h}: 0%` — тижневе вікно скидається через 1 день 2 години. Ти завжди знаєш, коли повернешся до 0%, тож можеш планувати важку роботу одразу після скидання й розподіляти свою продуктивність замість того, щоб посеред задачі впертися в ліміт. Працює на основі `rate_limits.*.resets_at`, який надсилає Claude Code; якщо твоя збірка не надсилає мітки часу скидання, лічильники плавно повертаються до простого вигляду `5h: 1%`.

## 🎨 164 готових варіанти — обирайте і застосовуйте

82 теми × 2 варіанти (`detailed` + `-compact`). Застосовуйте через `~/.claude/status-line.sh use <name>`.

### 🔝 Топ-10 — найпопулярніші, крос-культурні
`cyberpunk` · `hacker` · `dragonball` · `naruto` · `pokemon` · `ironman` · `spiderman` · `einstein` · `tesla` · `ferrari`

### 🛠 Класика (21): `minimal` `developer` `muted` `mono` `hard-worker` `time` `zen` `rainbow` `anime` `love` `cat` `christmas` `space` `retro` `fire` `ocean` `weather` `coffee` `music` `game` `pirate`
...
> 💡 `refreshInterval: 30` перезапускає рядок кожні 30 секунд навіть коли сесія простоює — підтримує живими зворотний відлік до скидання (5h{1h 6m}), трекер часу та перемикання станів після скидання. 30 — розумне значення за замовчуванням; 60 — ощадливіше до батареї; пропустіть, щоб оновлювати лише за подіями (нове повідомлення асистента, /compact, перемикання vim).
...
>    додай блок з абсолютним шляхом до скрипта та полем
>    `"refreshInterval": 30`. Якщо `statusLine` уже
...
| Зворотний відлік до скидання в лічильниках лімітів (`5h{1h 6m}`) | ❌ | ✅ |
| Git-гілка + dirty + ahead/behind | ❌ | ✅ |
| Час на задачі (active vs wall) | ❌ | ✅ |
| Рівень thinking / effort | ❌ | ✅ |
| Готові теми | ❌ | ✅ 82 теми × 2 варіанти = **164 варіанти** |
...
### Що означає `5h{1h 6m}: 1%`?

Ти використав 1% від 5-годинного вікна, а `{1h 6m}` — це живий зворотний відлік: вікно скидається через 1 годину 6 хвилин (`7d{1d 2h}`: тижневе вікно скидається через 1 день 2 години). Читається з `rate_limits.*.resets_at` на кожному рендері. Немає мітки часу скидання у твоїй збірці? Лічильник повертається до простого вигляду `5h: 1%`.

### Чи оновлюється статусний рядок сам по собі? Мій зворотний відлік `{1h 6m}` виглядає замороженим.

Claude Code перемальовує рядок за подіями — нове повідомлення асистента, `/compact`, зміна режиму дозволів чи режиму vim (з дебаунсом 300 мс) — тож між подіями рядок завмирає. Додай `"refreshInterval": 30` до блоку `statusLine` у `~/.claude/settings.json`, і він також перезапускатиметься за фіксованим 30-секундним таймером, підтримуючи живими зворотний відлік і трекер часу під час простою. Рендер коштує ~0.1 с, тож 30 с — мізерна вартість; використовуй 60 на батареї або у величезних репозиторіях (`git status` запускається на кожному рендері); мінімум — 1.
...
Так, трьома способами: (1) вибрати з **164 готових варіантів** у [`examples/`](examples/), (2) зібрати свій з іменованих блоків — див. [BLOCKS.md](BLOCKS.md), (3) редагувати кольорові константи в `statusline.sh` напряму.
