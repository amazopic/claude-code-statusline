#!/usr/bin/env bash
# ─────────────────────────────────────────────────────────────────────────
# statusline-time-compact.sh — compact time variant (model · ctx · branch)
# Author: Yevgeniy Achin <amazopic@gmail.com>
# Preview: Opus 4.7 · 12% · ⏱ 1h23m · main
# ─────────────────────────────────────────────────────────────────────────
G=$'\e[1;38;5;220m'; GR=$'\e[1;38;5;46m'; C=$'\e[1;38;5;51m'; B=$'\e[1;38;5;39m'; D=$'\e[38;5;244m'
...
export LC_NUMERIC=C
input=$(cat)
LIM=$' \e[38;5;240m·\e[0m \e[38;5;75m5h{1h 6m}:\e[0m \e[1;38;5;226m15%\e[0m \e[38;5;240m·\e[0m \e[38;5;75m7d{1d 2h}:\e[0m \e[1;38;5;226m5%\e[0m'
N=$'\e[0m'
...
fmt_dur(){ local s=$1; (( s<0 )) && s=0; local h=$((s/3600)) m=$(((s%3600)/60)); if (( h>0 )); then printf "%dh%02dm" $h $m; elif (( m>0 )); then printf "%dm" $m; else printf "%ds" $s; fi; }
to_ep(){ local t="$1"; [[ -z "$t" ]] && { echo 0; return; }; [[ "$t" =~ ^[0-9]+$ ]] && { echo "$t"; return; }; local c="${t%%.*}"; c="${c%Z}"; c="${c//T/ }"; date -j -f "%Y-%m-%d %H:%M:%S" "$c" +%s 2>/dev/null || date -d "$t" +%s 2>/dev/null || echo 0; }

active=0; first=0; last=0; prev=0
if [[ -n "$transcript" && -f "$transcript" ]]; then
  while IFS= read -r ts; do [[ -z "$ts" ]] && continue; e=$(to_ep "$ts"); (( e==0 )) && continue; (( first==0 )) && first=$e; last=$e; if (( prev>0 )); then g=$((e-prev)); (( g>0 && g<300 )) && active=$((active+g)); fi; prev=$e; done < <(jq -r ".timestamp // .created_at // .message.created_at // empty" "$transcript" 2>/dev/null)
fi
line="${G}${model_name}${N} ${D}·${N} ${GR}${ctx_pct}%${N} ${D}·${N} ${C}⏱ $(fmt_dur $active)${N}"
