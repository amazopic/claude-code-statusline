#!/usr/bin/env bash
# ─────────────────────────────────────────────────────────────────────────
# statusline-pirate-compact.sh — compact pirate variant (model · ctx · branch)
# Author: Yevgeniy Achin <amazopic@gmail.com>
# Preview: 🏴‍☠️ Opus 4.7 · 12% [█---------] · ⚓ main
# ─────────────────────────────────────────────────────────────────────────
GD=$'\e[1;38;5;220m'; B=$'\e[38;5;94m'; W=$'\e[1;38;5;255m'; D=$'\e[38;5;244m'
...
export LC_NUMERIC=C
input=$(cat)
LIM=$' ⚓ \e[1;38;5;130m5h{1h 6m}:\e[0m \e[1;38;5;220m15%\e[0m \e[38;5;215m·\e[0m \e[1;38;5;130m7d{1d 2h}:\e[0m \e[1;38;5;220m5%\e[0m'
N=$'\e[0m'
...
bar_p(){ local c=$(( ($1+5)/10 )); local s="" i; for ((i=0;i<c;i++));do s+="${GD}█";done; for ((i=0;i<10-c;i++));do s+="${B}-";done; printf "%s%s" "$s" "$N"; }

line="🏴‍☠️ ${W}${model_name}${N} ${D}·${N} ${GD}${ctx_pct}%${N} $(bar_p $ctx_pct)"
[[ -n "$br" ]] && line+=" ${D}·${N} ⚓ ${GD}${br}${N}"
