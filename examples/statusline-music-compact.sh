#!/usr/bin/env bash
# ─────────────────────────────────────────────────────────────────────────
# statusline-music-compact.sh — compact music variant (model · ctx · branch)
# Author: Yevgeniy Achin <amazopic@gmail.com>
# Preview: 🎵 Opus 4.7 · ♩───────── 12% · main
# ─────────────────────────────────────────────────────────────────────────
P=$'\e[1;38;5;201m'; G=$'\e[1;38;5;220m'; W=$'\e[1;38;5;255m'; D=$'\e[38;5;238m'; DG=$'\e[38;5;244m'
...
bar_m(){ local c=$(( ($1+5)/10 )); local notes=("♩" "♪" "♫" "♬" "♩" "♪" "♫" "♬" "♩" "♪"); local s="" i; for ((i=0;i<c;i++));do s+="${P}${notes[$i]}";done; for ((i=0;i<10-c;i++));do s+="${D}─";done; printf "%s%s" "$s" "$N"; }

line="🎵 ${W}${model_name}${N} ${DG}·${N} $(bar_m $ctx_pct) ${P}${ctx_pct}%${N}"
[[ -n "$br" ]] && line+=" ${DG}·${N} ${G}♩ ${br}${N}"
