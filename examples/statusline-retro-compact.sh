#!/usr/bin/env bash
# ─────────────────────────────────────────────────────────────────────────
# statusline-retro-compact.sh — compact retro variant (model · ctx · branch)
# Author: Yevgeniy Achin <amazopic@gmail.com>
# Preview: ▀▄ OPUS-4.7 ▰▰░░░░░░░░ 12% main
# ─────────────────────────────────────────────────────────────────────────
GR=$'\e[1;38;5;46m'; A=$'\e[1;38;5;208m'; W=$'\e[1;38;5;255m'; B=$'\e[1;38;5;39m'; D=$'\e[38;5;238m'
...
export LC_NUMERIC=C
input=$(cat)
LIM=$' \e[38;5;46m█\e[0m \e[38;5;46m5h{1h 6m}:\e[0m \e[1;38;5;226m15%\e[0m \e[38;5;46m█\e[0m \e[38;5;46m7d{1d 2h}:\e[0m \e[1;38;5;226m5%\e[0m'
N=$'\e[0m'
...
bar_r(){ local c=$(( ($1+5)/10 )); local s="" i; for ((i=0;i<c;i++));do s+="${GR}▰";done; for ((i=0;i<10-c;i++));do s+="${D}▱";done; printf "%s%s" "$s" "$N"; }
chrome(){ printf "%s" "$1" | tr "[:lower:] " "[:upper:]-"; }

cn=$(chrome "$model_name")
line="${A}▀▄${N} ${W}${cn}${N} $(bar_r $ctx_pct) ${B}${ctx_pct}%${N}"
[[ -n "$br" ]] && line+=" ${A}${br}${N}"
