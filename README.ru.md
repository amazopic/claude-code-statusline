# 🛰️ Claude Code Status Line — 82 темы, настраиваемые блоки, CLI

> Красивая, расширяемая статусная строка для Claude Code — контекст, лимиты, стоимость, модель и многое другое — всё в одной цветной линии. 82 темы и библиотека из 26 блоков для сборки собственной строки.
...
### ⏳ Reset countdown — планируй вокруг своих лимитов

**Предсказуемость работы — распределяй свою продуктивность.** Предсказуемость заложена в дизайн: каждый индикатор отсчитывает время до своего сброса, поэтому ты задаёшь себе ритм, а не упираешься в стену.

Индикаторы 5h / 7d включают живой обратный отсчёт до момента сброса каждого окна: `5h{1h 6m}: 1%` — 5-часовое окно сбросится через 1 час 6 минут; `7d{1d 2h}: 0%` — недельное окно сбросится через 1 день 2 часа. Ты всегда знаешь, когда снова окажешься на 0%, поэтому можешь запланировать тяжёлую работу сразу после сброса и распределить свою продуктивность вместо того, чтобы врезаться в потолок посреди задачи. Питается от `rate_limits.*.resets_at`, который присылает Claude Code; если твоя сборка не присылает таймстемпы сброса, индикаторы плавно откатываются к обычному `5h: 1%`.

## 🎨 164 готовых варианта — выбирайте и применяйте

82 темы × 2 варианта (`detailed` + `-compact`). Применяйте через `~/.claude/status-line.sh use <name>`.
...
### 🛠 Классика (21): `minimal` `developer` `muted` `mono` `hard-worker` `time` `zen` `rainbow` `anime` `love` `cat` `christmas` `space` `retro` `fire` `ocean` `weather` `coffee` `music` `game` `pirate`

### 🚗 Автомобильные бренды (16)
...
> 1. Если файл `~/.claude/status-line.sh` уже существует — забекапь его в
>    `~/.claude/status-line.sh.bak.<YYYYMMDD-HHMMSS>` (если такой бэкап
...
chmod +x ~/.claude/status-line.sh
~/.claude/status-line.sh use cyberpunk          # или: anime, hacker, minimal, …
```
...
| Обратный отсчёт до сброса в индикаторах лимитов (5h{1h 6m}) | ❌ | ✅ |
| Git-ветка + dirty + ahead/behind | ❌ | ✅ |
| Время на задаче (active vs wall) | ❌ | ✅ |
| Уровень thinking / effort | ❌ | ✅ |
| Готовые темы | ❌ | ✅ 82 темы × 2 варианта = **164 варианта** |
| Сборка из именованных блоков | ❌ | ✅ 26 блоков, см. [BLOCKS.md](BLOCKS.md) |
...
### Что означает `5h{1h 6m}: 1%`?

Ты использовал 1% 5-часового окна, а `{1h 6m}` — это живой обратный отсчёт: окно сбросится через 1 час 6 минут (`7d{1d 2h}`: недельное окно сбросится через 1 день 2 часа). Читается из `rate_limits.*.resets_at` при каждом рендере. В твоей сборке нет таймстемпа сброса? Индикатор откатывается к обычному `5h: 1%`.

### Обновляется ли статусная строка сама? Мой обратный отсчёт {1h 6m} выглядит замороженным.
...
Скопировать `statusline-bundle.sh` в `~/.claude/status-line.sh`, сделать `chmod +x`, прописать путь в `~/.claude/settings.json` → `statusLine.command`. Полные инструкции в разделах [Быстрый старт](#-быстрый-старт) и Установка.
...
Да, тремя способами: (1) выбрать из **164 готовых вариантов** в [`examples/`](examples/), (2) собрать свою из именованных блоков — см. [BLOCKS.md](BLOCKS.md), (3) править цветовые константы в `statusline.sh` напрямую.
...
Удалите блок `statusLine` из `~/.claude/settings.json` или выполните `~/.claude/status-line.sh reset`.
