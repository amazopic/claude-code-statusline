...
import { themes, blocks, faq, compare } from './themes.js?v=5';
import { ansiToHtml, specimenHtml, loadSpecimen } from './ansi.js?v=5';
import { messages, supportedLocales, defaultLocale, t as tBase, detectLocale, persistLocale } from './i18n.js?v=5';
...
let currentLocale = ((() => {
  ...
  if (fromQuery && messages[fromQuery]) {
...
async function runHeroAnimation() {
  ...
  const cycle = async () => {
    ...
    const fullCmd = `~/.claude/statusline.sh use ${themeName}`;
...
// ═════════════════════════════════════════════════════════════════════
//  CATALOG — render all 40 themes as TOC, click to jump to specimen
// ═════════════════════════════════════════════════════════════════════
...
function generateBash(ids) {
  const lines = [
    ...
    `# Blocks: ${ids.join(' · ')}`,
    '#',
    '# Drop into ~/.claude/statusline.sh and chmod +x.',
    '',
    'set -uo pipefail',
    'input=$(cat)',
    '',
    ...
    `~/.claude/statusline.sh custom ${ids.join(' ')}`,
  ];
  return lines.join('\n');
}
