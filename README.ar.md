...
تتضمن مؤشرات 5س / 7ي عدًّا تنازليًا مباشرًا حتى لحظة إعادة ضبط كل نافذة: `5h{1h 6m}: 1%` — تُعاد نافذة الـ5 ساعات خلال ساعة واحدة و6 دقائق؛ `7d{1d 2h}: 0%` — تُعاد النافذة الأسبوعية خلال يوم واحد وساعتين. أنت تعرف دائمًا متى تعود إلى 0%، فتستطيع جدولة العمل الثقيل مباشرةً بعد إعادة الضبط وتوزيع إنتاجيتك بدلًا من الاصطدام بالسقف في منتصف المهمة. مدعوم بـ `rate_limits.*.resets_at` الذي يرسله Claude Code؛ وإذا لم تُرسل نسختك طوابع زمنية لإعادة الضبط، تعود المؤشرات بسلاسة إلى الصيغة البسيطة `5h: 1%`.
...
> 💡 `refreshInterval: 30` يعيد تشغيل السطر كل 30 ثانية حتى أثناء خمول الجلسة — وهذا يُبقي العدّ التنازلي لإعادة الضبط (`5h{1h 6m}`) ومتتبع الوقت وتحوّلات ما بعد إعادة الضبط حيّة. القيمة 30 خيار افتراضي معقول؛ والقيمة 60 أوفر للبطارية؛ واحذف الحقل لتحديث السطر عند الأحداث فقط (رسالة مساعد جديدة، `/compact`، تبديل vim).
...
## 🎨 164 متغيرًا جاهزًا — اختر وانطلق

82 سمة × متغيرين (`detailed` + `-compact`). طبّق عبر `~/.claude/status-line.sh use <name>`.

### 🔝 أبرز 10 — الأكثر طلبًا، عبر الثقافات
`cyberpunk` · `hacker` · `dragonball` · `naruto` · `pokemon` · `ironman` · `spiderman` · `einstein` · `tesla` · `ferrari`

### 🛠 الكلاسيكية (21): `minimal` `developer` `muted` `mono` `hard-worker` `time` `zen` `rainbow` `anime` `love` `cat` `christmas` `space` `retro` `fire` `ocean` `weather` `coffee` `music` `game` `pirate`
...
### ماذا يعني `5h{1h 6m}: 1%`؟

لقد استخدمت 1% من نافذة الـ5 ساعات، و`{1h 6m}` عدّ تنازلي مباشر — تُعاد النافذة خلال ساعة واحدة و6 دقائق (`7d{1d 2h}`: تُعاد النافذة الأسبوعية خلال يوم واحد وساعتين). يُقرأ من `rate_limits.*.resets_at` في كل عرض. لا يوجد طابع زمني لإعادة الضبط في نسختك؟ يعود المؤشر بسلاسة إلى الصيغة البسيطة `5h: 1%`.

### هل يُحدِّث شريط الحالة نفسه تلقائيًا؟ يبدو العدّ التنازلي `{1h 6m}` مجمَّدًا.
...
1. اختر إحدى **82 سمة** (164 متغيرًا إجمالًا) — استخدم `~/.claude/status-line.sh use <name>` أو تصفّح [`examples/`](examples/) للسكريبتات المستقلة.
