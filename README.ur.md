...
| حد میٹرز میں ری سیٹ کاؤنٹ ڈاؤن (`5h{1.1h}`) | ❌ | ✅ |
...
### `5h{1.1h}: 1%` کا کیا مطلب ہے؟

آپ نے 5-گھنٹے کی window کا 1% استعمال کیا ہے، اور `{1.1h}` ایک لائیو کاؤنٹ ڈاؤن ہے — window 1.1 گھنٹے میں ری سیٹ ہوتی ہے (`7d{1.1d}`: ہفتہ وار window 1.1 دن میں ری سیٹ ہوتی ہے)۔ ہر رینڈر پر `rate_limits.*.resets_at` سے پڑھا جاتا ہے۔ آپ کے بِلڈ میں کوئی ری سیٹ ٹائم اسٹیمپ نہیں؟ میٹر سادہ `5h: 1%` پر واپس آ جاتا ہے۔

### کیا status line خود بخود اپ ڈیٹ ہوتی ہے؟ میرا `{1.1h}` کاؤنٹ ڈاؤن منجمد لگتا ہے۔
